...
class Agent:
    ...
    def __init__(self, stateSize, actionSize):
        ...
        self.loadEpisodeFrom = 6324  # Start to learn from this episode
    ...
    def calcQ(self, reward, nextStateQVal, done):
        """
        QL:
        ...
        if done:
            ...
            return reward + self.discountFactor * np.amax(nextStateQVal)
    ...
    def trainModel(self, target=False):
        ...
        for i in range(self.batchSize):
            ...
            if target:
                nextStateQVal = self.targetModel.predict(nextState.reshape(1, len(nextState)))

            else:
                nextStateQVal = self.model.predict(nextState.reshape(1, len(nextState)))

            targetTruth = self.calcQ(reward, nextStateQVal, done)

            xBatch = np.append(xBatch, np.array([state.copy()]), axis=0)
            ySample = qValue.copy()

            ySample[0][action] = targetTruth
